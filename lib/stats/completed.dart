    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (snap.isEmpty) {
            return Nodata(
                message: isEmployeeType ? "You havent completed any work yet." :  "No completed work yet.");
          }
    ...
    return FadeInLeftBig(
      child: Slidable(
        ...
        child: Card(
          elevation: 5.0,
          child: ListTile(
            title: Text(isEmployeeType ? data['EmployersFullName'] : data['FullName']),
            subtitle: RichText(
              maxLines: 2,
              overflow: TextOverflow.ellipsis,
              text: TextSpan(
                children: [
                  TextSpan(text:"Work completed",style:TextStyle(color:isDarkMode ?Colors.white:Colors.black) ),
                ],
              ),
            ),
            leading:
                CircleAvatar(backgroundColor:AppColors.kPrimaryColorTwo, child: Text(isEmployeeType ? data['EmployersFullName'][0].toUpperCase() : data['FullName'][0].toUpperCase(),style:const TextStyle(color:Colors.white)),),
            dense: false,
            trailing: Container(
              ...
              child: const Padding(
                padding:EdgeInsets.only(top: 3, bottom: 8, left: 10, right: 10),
                child: Text(
                  "Rejected",
                  style: TextStyle(
                      color: AppColors.kPrimaryColorTwo,
